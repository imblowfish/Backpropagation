...
class BMPParser:
	def from_bmp_to_array(path):
		image = Image.open(path)
		image_size = 55
		x1 = 20
		y1 = 253
		x2 = x1 + image_size * 4 + 33
		y2 = y1 + image_size
		# image.crop((x1, y1, x2, y2)).show()
		images = []
		images.append(image.crop((x1, y1, x1+image_size, y1+image_size)))
		x1 += image_size + 13
		images.append(image.crop((x1, y1, x1+image_size, y1+image_size)))
		x1 += image_size + 15
		images.append(image.crop((x1, y1, x1+image_size, y1+image_size)))
		x1 += image_size + 5
		images.append(image.crop((x1, y1, x1+image_size, y1+image_size)))
		#преобразование изображений в бинарную матрицу
		binary_images = []
		for image in images:
			data = image.getdata()
			binary_image = []
